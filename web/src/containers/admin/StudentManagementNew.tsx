...
const StudentManagement = () => {
  ...
  // console.log("editStudent.date_of_birth",new Date(editStudent.date_of_birth))
  ...
  const handleEditStudent = async () => {
    ...
    // Email uniqueness check excluding current student
    ...
    try {
      // Prepare the data for sending to backend
      const updateData = {
        ...editStudent,
        batch_id: editStudent.batch_id || undefined,
  course_id: editStudent.course_id || undefined,
  community: editStudent.community || undefined
      };
  ...
  const handleAddStudent = async () => {
    ...
    if (!newStudent.enrollment_number) extraErrors.push('Enrollment number not generated');
    ...
    setFieldErrors(errs);
    // Email uniqueness check for added
  ...
  useEffect(() => {
    ...
    const sanitizedBatch = (batch.batch_name || '').replace(/\s+/g, '').toUpperCase();
    const newPrefix = `AVP${year}${sanitizedBatch}`; // New format without hyphens
    const legacyNoDashBeforeNum = `AVP${year}-${sanitizedBatch}`; // legacy variants for continuity
    const legacyWithDashBeforeNum = `${legacyNoDashBeforeNum}-`;
    // Find existing enrollment numbers with any recognized prefix
    const related = students
      ...
      .filter((en: string | undefined) => en && (
        ...
        en.startsWith(legacyWithDashBeforeNum) ||
        en.startsWith(legacyNoDashBeforeNum)
      )) as string[];
    let maxNum = 0;
    related.forEach(en => {
      const match = en.match(/^(.*?)(\d+)$/); // capture trailing digits
      if (match) {
    ...
    const enrollment_number = `${newPrefix}${nextStr}`; // AVP<year><BATCH><NNN>
    setNewStudent(prev => ({ ...prev, enrollment_number }));
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [newStudent.batch_id, isAddDialogOpen, students]);

  // Auto-generate enrollment number for edit dialog if batch changes (readonly field)
  useEffect(() => {
    if (!isEditDialogOpen || !editStudent?.batch_id) return;
    const batch = batches.find(b => b.batch_id.toString() === editStudent.batch_id);
    if (!batch) return;
    const year = new Date().getFullYear();
    const sanitizedBatch = (batch.batch_name || '').replace(/\s+/g, '').toUpperCase();
    const newPrefix = `AVP${year}${sanitizedBatch}`;
    const legacyNoDashBeforeNum = `AVP${year}-${sanitizedBatch}`;
    const legacyWithDashBeforeNum = `${legacyNoDashBeforeNum}-`;
    const related = students
      .map(s => s.student_profile?.enrollment_number || s.enrollment_number)
      .filter((en: string | undefined) => en && (
  ...
  }, [editStudent?.batch_id, isEditDialogOpen, students, batches]);
